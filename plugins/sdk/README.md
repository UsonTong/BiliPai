...
> UI 美化请不要打包成 `.bpplugin`。首页顶部氛围、搜索框和底栏饰面走 `.bpskin` 资源包；`.bpskin` 不执行代码，也不能替换底栏液态玻璃、指示器或滑动色散链路。可参考 [`plugins/samples/winter-cloud-skin`](../samples/winter-cloud-skin) 打包一个可导入的冬日云朵皮肤。
