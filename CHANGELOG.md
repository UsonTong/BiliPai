...
### 更新内容
- **个人页与首页体验**：当底栏已有“历史”入口时，“我的”页会隐藏重复历史按钮；“我的收藏”下新增收藏夹快捷入口；首页顶部标签在少量分类开启时自动居中。
- **视频与评论体验**：视频介绍默认展开；进入视频自动播放开关的文案和搜索命中更清晰；评论回复预览数量可在播放设置中调整，减少频繁点展开。
- **图片分享**：图片预览新增系统分享按钮，评论区图片打开预览后可直接分享，GIF/WebP/PNG/JPEG 会尽量保留原格式。
- **播放与搜索细节**：补充番剧评论入口、播放流选择策略和评论/动态搜索相关策略，优化动态图片预览动效与交互反馈。
- **直播与数据解析**：完善直播间人数、关注直播数据和直播面板显示逻辑，降低空数据和布局状态异常。
- **回归覆盖**：补充个人页服务隐藏、收藏夹快捷入口、顶部标签居中、视频介绍展开、评论预览数量、图片分享格式、直播解析和播放流选择等策略测试。
